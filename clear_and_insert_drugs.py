#!/usr/bin/env python3
"""
Script untuk menghapus semua obat yang ada dan menginsert 37 obat baru
untuk apotek retail di Indonesia.
"""
...
def clear_existing_drugs(connection):
    """Hapus semua data dari tabel obat"""
    try:
        cursor = connection.cursor()
        cursor.execute("DELETE FROM obat")
        connection.commit()
        print(f"✅ Berhasil menghapus {cursor.rowcount} baris dari tabel obat")
        cursor.close()
        return True
    except Error as e:
        print(f"Error deleting from obat table: {e}")
        return False

def insert_new_drugs(connection):
    """Insert 37 obat baru"""
    new_drugs = [
    ...
    try:
        ...
        print(f"✅ Berhasil menginsert {len(new_drugs)} obat baru ke tabel obat")
        cursor.close()
        return True
...
def main():
    print("=== Script Hapus dan Insert Obat ===")
    print("Script ini akan menghapus semua obat yang ada dan menginsert 37 obat baru")
    print("untuk apotek retail di Indonesia.")
    print()
    
    # Connect to database
    connection = connect_to_database()
    if not connection:
        print("❌ Gagal terhubung ke database. Script dihentikan.")
        sys.exit(1)
    
    try:
        # Clear existing drugs
        if clear_existing_drugs(connection):
            # Insert new drugs
            if insert_new_drugs(connection):
                print()
                print("✅ === Operasi Berhasil ===")
                print("✅ Semua obat yang ada telah dihapus")
                print("✅ 37 obat baru telah diinsert")
                print("✅ Database siap digunakan")
                print()
                print("Anda sekarang dapat:")
                print("1. Run 'php artisan serve' untuk memulai server")
                print("2. Akses aplikasi di: http://localhost:8000")
                print("3. Gunakan kredensial default: admin@pharmacy.local / password")
            else:
                print("❌ Gagal menginsert obat baru")
                sys.exit(1)
        else:
            print("❌ Gagal menghapus obat yang ada")
            sys.exit(1)
    finally:
        connection.close()
