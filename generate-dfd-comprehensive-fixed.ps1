...
function AddProcess($writer, $x, $y, $pw, $ph, $label, $fill, $stroke) {
    $id = NextId; $l = esc $label; $f = esc $fill; $s = esc $stroke
    $writer.WriteLine("        <mxCell id=`"$id`" value=`"$l`" style=`"rounded=1;whiteSpace=wrap;html=1;fillColor=$f;strokeColor=$s;fontSize=11;`" parent=`"1`" vertex=`"1`">")
    $writer.WriteLine("          <mxGeometry x=`"$x`" y=`"$y`" width=`"$pw`" height=`"$ph`" as=`"geometry`" />")
    $writer.WriteLine("        </mxCell>")
    return $id
}

function AddEntity($writer, $x, $y, $pw, $ph, $label) {
    $id = NextId; $l = esc $label
    $writer.WriteLine("        <mxCell id=`"$id`" value=`"$l`" style=`"rounded=0;whiteSpace=wrap;html=1;shadow=1;fillColor=#FFF2CC;strokeColor=#D6B656;fontSize=11;`" parent=`"1`" vertex=`"1`">")
    $writer.WriteLine("          <mxGeometry x=`"$x`" y=`"$y`" width=`"$pw`" height=`"$ph`" as=`"geometry`" />")
    $writer.WriteLine("        </mxCell>")
    return $id
}

function AddStore($writer, $x, $y, $pw, $ph, $label) {
    $id = NextId; $l = esc $label
    $writer.WriteLine("        <mxCell id=`"$id`" value=`"$l`" style=`"shape=cylinder;whiteSpace=wrap;html=1;fillColor=#E1D5E7;strokeColor=#9673A6;fontSize=11;`" parent=`"1`" vertex=`"1`">")
    $writer.WriteLine("          <mxGeometry x=`"$x`" y=`"$y`" width=`"$pw`" height=`"$ph`" as=`"geometry`" />")
    $writer.WriteLine("        </mxCell>")
    return $id
}

function AddFlow($writer, $from, $to, $exX, $exY, $enX, $enY, $label) {
    $id = NextId; $l = esc $label
    ...
    $writer.WriteLine("        <mxCell id=`"$id`" value=`"$l`" style=`"$style`" parent=`"1`" source=`"$from`" target=`"$to`" edge=`"1`">")
    $writer.WriteLine("          <mxGeometry relative=`"1`" as=`"geometry`" />")
    $writer.WriteLine("        </mxCell>")
}

function NewDiagram($name, $writer, $pw, $ph) {
    $writer.WriteLine('  <diagram id="' + $name + '" name="' + $name + '">')
    $writer.WriteLine('    <mxGraphModel dx="0" dy="0" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="' + $pw + '" pageHeight="' + $ph + '" math="0" shadow="0">')
    $writer.WriteLine('      <root>')
    $writer.WriteLine('        <mxCell id="0" />')
    $writer.WriteLine('        <mxCell id="1" parent="0" />')
}

function EndDiagram($writer) {
    $writer.WriteLine('      </root>')
